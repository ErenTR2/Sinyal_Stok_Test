...
export default function Outbound(){
  ...
  const [form, setForm] = useState({ tarih: new Date().toISOString().slice(0,10), depo:"", musteri:"", aciklama:"" });
  ...
  const submit = (e) => {
    e.preventDefault();
    // TODO: backend POST /outbound { ...form, lines }
    alert("Çıkış kaydedildi (demo).");
    setForm({ tarih: new Date().toISOString().slice(0,10), depo:"", musteri:"", aciklama:"" });
      ...
      <form className="mt-3" onSubmit={submit}>
        <div className="row g-2">
          ...
          <div className="col-md-5">
            <label className="form-label">Müşteri/Proje</label>
            <input className="form-control" value={form.musteri} onChange={e=>setForm({...form,musteri:e.target.value})}/>
          </div>
        </div>
